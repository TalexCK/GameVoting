...
/**
 * Singleton manager for player boss bars. Handles creation, display, update, and cleanup of boss
 * bars.
 *
 * <p>Features: - Per-player boss bar tracking - Timed boss bars with automatic removal -
 * Thread-safe operations - Auto-cleanup on player disconnect - Color and style support
 */
public class BossBarManager {
  ...
  public void showBar(Player player, String text, BossBar.Color color, float progress) {
    ...
    if (isDebugEnabled()) {
      GameVoting.getInstance()
          ...
          .info(
              String.format(
                  "[DEBUG] Showed boss bar to %s: %s (%.0f%%)",
  ...
  public void showTimedBar(Player player, String text, BossBar.Color color, int durationSeconds) {
    ...
    if (isDebugEnabled()) {
      GameVoting.getInstance()
          ...
          .info(
              String.format(
                  "[DEBUG] Showed timed boss bar to %s for %d seconds",
  ...
  public void updateBar(Player player, String text, float progress) {
    ...
    if (isDebugEnabled()) {
      GameVoting.getInstance()
          ...
          .info(
              String.format(
                  "[DEBUG] Updated boss bar for %s: %.0f%%", player.getName(), progress * 100));
    }
  }
  ...
  public void removeBar(Player player) {
    ...
    if (bossBar != null) {
      ...
      if (isDebugEnabled()) {
        GameVoting.getInstance()
            .getLogger()
            .info(String.format("[DEBUG] Removed boss bar from %s", player.getName()));
      }
    }
  }
  ...
  /** Shutdown the manager and clean up all active boss bars. Called on plugin disable. */
