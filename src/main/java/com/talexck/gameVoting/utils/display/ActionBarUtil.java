...
/**
 * Utility class for sending action bar messages to players. Action bars appear above the player's
 * hotbar.
 *
 * <p>Supports: - Colored messages (& codes and MiniMessage format) - Simple one-line text display -
 * Optional debug logging
 */
public class ActionBarUtil {

  /**
   * Send an action bar message to a player. The message appears above the hotbar and fades after a
   * few seconds.
   *
  ...
  public static void sendActionBar(String message, Player... players) {
    ...
    if (isDebugEnabled()) {
      GameVoting.getInstance()
          ...
          .info(
              String.format(
                  "[DEBUG] Sent action bar to %d players: %s",
  ...
  public static void clearActionBar(Player player) {
    ...
    if (isDebugEnabled()) {
      GameVoting.getInstance()
          .getLogger()
          .info(String.format("[DEBUG] Cleared action bar for %s", player.getName()));
    }
  }
