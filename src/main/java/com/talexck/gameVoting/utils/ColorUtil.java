...
/**
 * Utility class for handling color codes and text formatting. Converts legacy & color codes to
 * Minecraft § codes.
 *
 * <p>Compatible with both legacy Paper (1.16.5) and modern Paper (1.20+). Falls back to legacy
 * color codes if MiniMessage is not available.
 */
public class ColorUtil {
   ...
   *
   * <p>Examples: - "&aGreen &c&lRed Bold" → colored text - "<#FF5555>Hex Color" → RGB color -
  ...
  public static Component colorize(String message) {
    ...
    try {
      ...
      if (MINI_MESSAGE_AVAILABLE && message.contains("<") && message.contains(">")) {
        ...
        return ((net.kyori.adventure.text.minimessage.MiniMessage) MINI_MESSAGE_INSTANCE)
            .deserialize(message);
      } else {
  ...
  /**
   * Colorize a string and return it as a plain text Component. Same as colorize() but with a more
   * explicit name.
   *
  ...
  /**
   * Strip all color codes from a string (both § and & codes). Useful for logging or storing clean
   * text.
   *
  ...
  /**
   * Convert a Component back to legacy format string with § codes. Useful for storing colored text
   * in configs.
   *
  ...
  public static String withReset(String text) {
    if (text == null || text.isEmpty() || text.endsWith("&r") || text.endsWith("§r")) {
      return text;
    }
    return text + "&r";
  }
