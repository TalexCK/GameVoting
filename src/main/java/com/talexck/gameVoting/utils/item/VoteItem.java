...
/**
 * Utility class for managing the vote hotbar item. The item is placed in slot 8 (9th slot) and
 * cannot be dropped.
 */
public class VoteItem {
  private static final int VOTE_ITEM_SLOT = 8;
  private static final int SOLO_ITEM_SLOT = 5;
  private static NamespacedKey VOTE_ITEM_KEY;
  private static final NamespacedKey GLOW_ENCHANTMENT_KEY =
    ...
    if (meta != null) {
      meta.setDisplayName(
          ChatColor.translateAlternateColorCodes(
              '&', langManager.getMessage("item.insufficient_players_name")));
      meta.setLore(
          Arrays.asList(
              ChatColor.translateAlternateColorCodes(
                  '&', langManager.getMessage("item.insufficient_players_lore_1")),
              ChatColor.translateAlternateColorCodes(
                  '&', langManager.getMessage("item.insufficient_players_lore_2")),
              ChatColor.translateAlternateColorCodes(
                  '&', langManager.getMessage("item.insufficient_players_lore_3", placeholders)),
              ChatColor.translateAlternateColorCodes(
                  '&', langManager.getMessage("item.insufficient_players_lore_4", placeholders))));

      addGlow(meta);

      // Mark as insufficient players item
      meta.getPersistentDataContainer()
          .set(VOTE_ITEM_KEY, PersistentDataType.STRING, "insufficient_players");
  ...
  public static void giveStartVotingItem(Player player) {
    ...
    if (meta != null) {
      meta.setDisplayName(
          ChatColor.translateAlternateColorCodes(
              '&', langManager.getMessage("item.start_voting_name")));
      meta.setLore(
          Arrays.asList(
              ChatColor.translateAlternateColorCodes(
                  '&', langManager.getMessage("item.start_voting_lore_1")),
              ChatColor.translateAlternateColorCodes(
                  '&', langManager.getMessage("item.start_voting_lore_2")),
              ChatColor.translateAlternateColorCodes(
                  '&', langManager.getMessage("item.start_voting_lore_3"))));

      addGlow(meta);

      // Mark as start voting item
      meta.getPersistentDataContainer()
          .set(VOTE_ITEM_KEY, PersistentDataType.STRING, "start_voting");
  ...
  public static void updateStartVotingItem(Player player, boolean isReady) {
    ...
    if (isReady) {
      ...
      if (meta != null) {
        meta.setDisplayName(
            ChatColor.translateAlternateColorCodes(
                '&', langManager.getMessage("item.already_ready_start_name")));
        meta.setLore(
            Arrays.asList(
                ChatColor.translateAlternateColorCodes(
                    '&', langManager.getMessage("item.already_ready_start_lore_1")),
                ChatColor.translateAlternateColorCodes(
                    '&', langManager.getMessage("item.already_ready_start_lore_2")),
                ChatColor.translateAlternateColorCodes(
                    '&', langManager.getMessage("item.already_ready_start_lore_3"))));
  ...
  public static void giveVotingItem(Player player) {
    ...
    if (meta != null) {
      meta.setDisplayName(
          ChatColor.translateAlternateColorCodes(
              '&', langManager.getMessage("item.voting_item_name")));
      meta.setLore(
          Arrays.asList(
              ChatColor.translateAlternateColorCodes(
                  '&', langManager.getMessage("item.voting_item_lore_1")),
              ChatColor.translateAlternateColorCodes(
                  '&', langManager.getMessage("item.voting_item_lore_2")),
              ChatColor.translateAlternateColorCodes(
                  '&', langManager.getMessage("item.voting_item_lore_3"))));
  ...
  public static void giveSoloItem(Player player) {
    var langManager = com.talexck.gameVoting.utils.language.LanguageManager.getInstance();
    ...
    if (meta != null) {
      meta.setDisplayName(
          ChatColor.translateAlternateColorCodes(
              '&', langManager.getMessage("item.solo_item_name")));
      meta.setLore(
          Arrays.asList(
              ChatColor.translateAlternateColorCodes(
                  '&', langManager.getMessage("item.solo_item_lore_1")),
              ChatColor.translateAlternateColorCodes(
                  '&', langManager.getMessage("item.solo_item_lore_2")),
              ChatColor.translateAlternateColorCodes(
                  '&', langManager.getMessage("item.solo_item_lore_3"))));
      addGlow(meta);
      meta.getPersistentDataContainer().set(VOTE_ITEM_KEY, PersistentDataType.STRING, "solo");
      item.setItemMeta(meta);
    }

    player.getInventory().setItem(SOLO_ITEM_SLOT, item);
  }
  ...
  public static void giveReadyItem(Player player) {
    ...
    if (meta != null) {
      meta.setDisplayName(
          ChatColor.translateAlternateColorCodes(
              '&', langManager.getMessage("item.ready_item_name")));
      meta.setLore(
          Arrays.asList(
              ChatColor.translateAlternateColorCodes(
                  '&', langManager.getMessage("item.ready_item_lore_1")),
              ChatColor.translateAlternateColorCodes(
                  '&', langManager.getMessage("item.ready_item_lore_2")),
              ChatColor.translateAlternateColorCodes(
                  '&', langManager.getMessage("item.ready_item_lore_3"))));
  ...
  public static void updateReadyItem(Player player, boolean isReady) {
    ...
    if (isReady) {
      ...
      if (meta != null) {
        meta.setDisplayName(
            ChatColor.translateAlternateColorCodes(
                '&', langManager.getMessage("item.already_ready_name")));
        meta.setLore(
            Arrays.asList(
                ChatColor.translateAlternateColorCodes(
                    '&', langManager.getMessage("item.already_ready_lore_1")),
                ChatColor.translateAlternateColorCodes(
                    '&', langManager.getMessage("item.already_ready_lore_2")),
                ChatColor.translateAlternateColorCodes(
                    '&', langManager.getMessage("item.already_ready_lore_3"))));
        meta.addItemFlags(ItemFlag.HIDE_ENCHANTS);
        meta.getPersistentDataContainer().set(VOTE_ITEM_KEY, PersistentDataType.STRING, "ready");
        item.setItemMeta(meta);
      }
  ...
  public static void removeSoloItem(Player player) {
    player.getInventory().setItem(SOLO_ITEM_SLOT, null);
  }

  private static void addGlow(ItemMeta meta) {
    Enchantment enchantment = Enchantment.getByKey(GLOW_ENCHANTMENT_KEY);
    if (enchantment != null) {
  ...
  public static int getSoloItemSlot() {
    return SOLO_ITEM_SLOT;
  }

  public static boolean isProtectedSlot(int slot) {
    return slot == VOTE_ITEM_SLOT || slot == SOLO_ITEM_SLOT;
  }
}
