...
/**
 * Singleton manager for holograms. Provides a simplified API for creating and managing persistent
 * holograms.
 *
 * <p>Features: - Automatic provider selection (DecentHolograms or NoOp) - Simplified hologram
 * creation with varargs support - Persistent holograms (saved to DecentHolograms config) - Graceful
 * degradation when DecentHolograms is not available
 */
public class HologramManager {
  ...
  /**
   * Initialize the HologramManager with the appropriate provider. Should be called in the plugin's
   * onEnable() method.
   *
  ...
  public static HologramManager getInstance() {
    if (!initialized) {
      throw new IllegalStateException(
          "HologramManager has not been initialized! Call initialize() first.");
    }
    return instance;
  }
  ...
  public boolean createHologram(String id, Location location, List<String> lines) {
    ...
    if (result && isDebugEnabled()) {
      GameVoting.getInstance()
          ...
          .info(
              String.format(
                  "[DEBUG] Created hologram '%s' at %s with %d lines",
  ...
  public boolean updateLines(String id, List<String> lines) {
    ...
    if (result && isDebugEnabled()) {
      GameVoting.getInstance()
          .getLogger()
          .info(String.format("[DEBUG] Updated hologram '%s' with %d lines", id, lines.size()));
    }
  ...
  public boolean deleteHologram(String id) {
    ...
    if (result && isDebugEnabled()) {
      GameVoting.getInstance().getLogger().info(String.format("[DEBUG] Deleted hologram '%s'", id));
    }
  ...
  /**
   * Shutdown the manager and optionally delete all holograms. Called on plugin disable.
   *
   * <p>Note: Since holograms are persistent, they are NOT deleted on shutdown unless explicitly
   * requested. To clean up all holograms, call deleteAll() first.
   */
  ...
  /**
   * Delete all holograms created by this plugin. WARNING: This will permanently remove all
   * GameVoting holograms from the config.
   *
  ...
  public int deleteAll() {
    ...
    if (deleted > 0 && isDebugEnabled()) {
      GameVoting.getInstance()
          .getLogger()
          .info(String.format("[DEBUG] Deleted %d holograms", deleted));
    }
