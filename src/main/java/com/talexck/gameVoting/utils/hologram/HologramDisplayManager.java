...
/**
 * Manager for hologram display content based on voting state. Updates hologram text dynamically as
 * voting progresses.
 */
public class HologramDisplayManager {
  ...
  public void updateAllHolograms(DisplayState state, List<HologramLocation> locations) {
    for (int i = 0; i < locations.size(); i++) {
      ...
      if (bukkitLoc == null) {
        plugin
            .getLogger()
            .warning("Hologram #" + i + " world not loaded: " + hologramLoc.getWorldName());
        continue;
      }
    ...
    if (dbManager != null && dbManager.hasVoteHistoryRepository()) {
      ...
      if (topGames.isEmpty()) {
        ...
        for (Map.Entry<String, Integer> entry : topGames.entrySet()) {
          ...
          String gameName = ColorUtil.withReset(gameConfig != null ? gameConfig.getName() : gameId);
  ...
  private List<String> generateVotingGameList() {
    ...
    List<GameConfig> availableGames =
        gamesManager.getAvailableGames(Bukkit.getOnlinePlayers().size());
    for (GameConfig game : availableGames) {
      lines.add("&f• &e" + ColorUtil.withReset(game.getName()));
    }
