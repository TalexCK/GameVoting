...
/**
 * Paper side proxy bridge for player version queries. Uses plugin message channel to fetch version
 * data from velocity plugin.
 */
public final class ProxyVersionBridge implements PluginMessageListener {
  ...
  public void shutdown() {
    ...
    versionCache.clear();
    for (CompletableFuture<String> request : pendingRequests.values()) {
  ...
  public CompletableFuture<Optional<String>> resolvePlayerVersion(
      Player tunnelPlayer, UUID targetPlayerId) {
    if (tunnelPlayer == null || targetPlayerId == null || !tunnelPlayer.isOnline()) {
      return CompletableFuture.completedFuture(Optional.empty());
    }
    String cached = versionCache.get(targetPlayerId);
    if (cached != null) {
    ...
    CompletableFuture<String> request =
        pendingRequests.computeIfAbsent(
            ...
            ignored -> {
              ...
              scheduleRequest(tunnelPlayer, targetPlayerId, created, 0L);
              scheduleRequest(tunnelPlayer, targetPlayerId, created, 2L);
              scheduleRequest(tunnelPlayer, targetPlayerId, created, 5L);
              scheduleRequest(tunnelPlayer, targetPlayerId, created, 10L);
              Bukkit.getScheduler()
                  .runTaskLater(
                      plugin,
                      () -> {
                        if (pendingRequests.remove(targetPlayerId, created)) {
    ...
    return request.thenApply(Optional::ofNullable);
  }
  ...
  private void scheduleRequest(
    ...
    Runnable action =
        () -> {
          if (!request.isDone() && tunnelPlayer.isOnline()) {
            requestPlayerVersion(tunnelPlayer, targetPlayerId);
          }
        };
    if (delayTicks == 0L) {
      action.run();
  ...
  public void invalidate(UUID playerId) {
    if (playerId != null) {
      ...
      CompletableFuture<String> request = pendingRequests.remove(playerId);
      if (request != null) {
        request.complete(null);
      }
    }
  ...
  public void onPluginMessageReceived(
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(message))) {
      ...
      CompletableFuture<String> request = pendingRequests.remove(target);
      if (request != null) {
