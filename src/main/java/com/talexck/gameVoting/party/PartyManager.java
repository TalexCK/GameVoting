...
/**
 * Singleton manager for all party operations. Handles party creation, dissolution, invitations, and
 * member management.
 */
public class PartyManager {
  ...
  private static final int MAX_PARTY_SIZE = 16;
  ...
  public boolean sendInvite(UUID inviterId, UUID targetId) {
    ...
    if (party == null || !party.canInvite()) return false;
  ...
  /**
   * Remove a player from their party. If the leader leaves, leadership is transferred to another
   * member. If the party becomes empty, it is disbanded.
   *
  ...
  /**
   * Handle a player quitting the server. Removes them from their party and cleans up invitations.
   *
   * @param playerId The player's UUID
   */
