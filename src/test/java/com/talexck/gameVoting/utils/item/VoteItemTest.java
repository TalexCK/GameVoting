 ...
 *
 * <p>Note: VoteItem has dependencies on LanguageManager and requires plugin initialization. These
 * tests verify the core functionality that can be tested without full plugin context: -
 * removeVoteItem() - Slot 8 manipulation - isVoteItem() with null/unmarked items
 *
 * <p>Full item creation and identification tests require integration testing in a running server
 * environment.
 */
class VoteItemTest {
  ...
  @Test
  @DisplayName("Should remove solo item from slot 5")
  void testRemoveSoloItem() {
    player.getInventory().setItem(5, new ItemStack(Material.GLOWSTONE_DUST));
    player.getInventory().setItem(8, new ItemStack(Material.REDSTONE_BLOCK));

    VoteItem.removeSoloItem(player);
  ...
  @Test
  @DisplayName("Should protect fixed hotbar slots 5 and 8")
  void testProtectedSlots() {
    assertTrue(VoteItem.isProtectedSlot(5));
    assertTrue(VoteItem.isProtectedSlot(8));
    assertFalse(VoteItem.isProtectedSlot(4));
    assertFalse(VoteItem.isProtectedSlot(6));
  }
  ...
  void testSlot8Manipulation() {
    ...
    assertEquals(
        Material.DIAMOND,
        player.getInventory().getItem(0).getType(),
        "Slot 0 should still have diamond");
    assertNotNull(player.getInventory().getItem(7), "Slot 7 should be unaffected");
    assertEquals(
        Material.EMERALD,
        player.getInventory().getItem(7).getType(),
        "Slot 7 should still have emerald");
  }
  ...
  void testIsVoteItemNoMeta() {
    ...
    assertFalse(
        VoteItem.isVoteItem(itemNoMeta), "Should return false for item without persistent data");
  }
