 ...
 *
 * <p>Note: Some functionality requires the GameVoting plugin instance for debug logging. These
 * tests focus on core boss bar management logic that doesn't depend on plugin internals.
 */
class BossBarManagerTest {
  ...
  void testShowBarNullPlayer() {
    assertDoesNotThrow(
        () -> manager.showBar(null, "Test", BossBar.Color.GREEN, 0.5f),
  ...
  void testShowBarNullText() {
    assertDoesNotThrow(
        () -> manager.showBar(player1, null, BossBar.Color.GREEN, 0.5f),
  ...
  void testProgressClamping() {
    ...
    assertTrue(
        manager.hasBar(player1), "Should create boss bar with negative progress clamped to 0");
  ...
  void testUpdateNonExistentBar() {
    ...
    assertTrue(
        manager.hasBar(player1), "Should create new boss bar when updating non-existent one");
  }
  ...
  void testUpdateProgress() {
    ...
    assertDoesNotThrow(
        () -> manager.updateProgress(player1, 0.8f), "Should update progress without error");
  ...
  void testUpdateText() {
    ...
    assertDoesNotThrow(
        () -> manager.updateText(player1, "Updated"), "Should update text without error");
  ...
  void testTimedBarInvalidDuration() {
    assertDoesNotThrow(
        () -> manager.showTimedBar(player1, "Test", BossBar.Color.GREEN, 0),
        "Should handle zero duration without error");
    assertDoesNotThrow(
        () -> manager.showTimedBar(player1, "Test", BossBar.Color.GREEN, -5),
